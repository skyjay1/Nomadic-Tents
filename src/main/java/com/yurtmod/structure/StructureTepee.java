...
public class StructureTepee extends StructureBase
{
	...
	@Override
	public void buildLayer(World worldIn, BlockPos doorPos, EnumFacing dirForward, IBlockState state, BlockPos[] coordinates)
	{
		for(BlockPos coord : coordinates)
		{
			...
			if(state.getBlock() instanceof BlockTepeeWall)
			{
				...
				if(pos.getY() % 2 == 1)
				{
					// psuedo-random seed ensures that all blocks that are same y-dis from door get the same seed
				...
				else tepeeState = BlockTepeeWall.getStateForRandomDesignWithChance(worldIn.rand);
				worldIn.setBlockState(pos, tepeeState, 3);				
			} else worldIn.setBlockState(pos, state, 3);
		}
	}
	...
	private static Blueprints makeBlueprints(final StructureType.Size size, Blueprints bp) {
		...
		case MEDIUM:
			bp.addWallCoords(new int[][] {
				...
				// layer 3
				{0,2,0},{1,2,1},{1,2,-1},{2,2,-2},{3,2,-2},{4,2,-2},{5,2,-1},{5,2,0},{5,2,1},{4,2,2},{3,2,2},{2,2,2},
				// layer 4
		...
		case SMALL:
			bp.addWallCoords(new int[][] {
				...
				// layer 3
				{0,2,0},{1,2,-1},{1,2,1},{2,2,1},{3,2,1},{3,2,0},{3,2,-1},{2,2,-1},
				// layer 4
