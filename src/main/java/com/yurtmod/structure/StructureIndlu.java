...
public class StructureIndlu extends StructureBase {
	...
	public boolean generate(World worldIn, BlockPosBeta doorBase, EnumFacing dirForward, Size size, Block doorBlock,
		...
		case LARGE:
			...
			if (tentDim) {
				BlockPosBeta pos = getPosFromDoor(doorBase, 4, 0, 0, dirForward);
				if (pos.isAirBlock(worldIn)) {
					pos.setBlock(worldIn, Blocks.torch, 0, 2);
				}
			}
			return true;
		case MEDIUM:
			...
			// add dimension-only features
			if (tentDim) {
				BlockPosBeta pos = getPosFromDoor(doorBase, 3, 0, 0, dirForward);
				if (pos.isAirBlock(worldIn)) {
					pos.setBlock(worldIn, Blocks.torch, 0, 2);
				}
			}
	...
	public boolean isValidForFacing(World worldIn, BlockPosBeta doorBase, Size size, EnumFacing facing) {
		...
		Predicate<Block> checkBlockPred = new Predicate<Block>() {
			...
			public boolean test(Block b) {
				return b instanceof IIndluBlock;
			}
		};
