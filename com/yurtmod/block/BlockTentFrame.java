...
public class BlockTentFrame extends BlockUnbreakable implements IFrameBlock
{
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		if(!worldIn.isRemote)
		{
			if(heldItem != null && heldItem.getItem() instanceof ItemMallet)
			{
				if(heldItem.getItem() == Content.ITEM_SUPER_MALLET)
				{
					return onSuperMalletUsed(worldIn, pos, state, heldItem, player);
				}
				else return onMalletUsed(worldIn, pos, state, heldItem, player);
			}
		}
		return false;
	}
...
*/
	...
	public static enum BlockToBecome
	{
		...
		public IBlockState getBlock()
		{
			switch(this)
			{
			case YURT_WALL_INNER: 	return Content.YURT_WALL_INNER.getDefaultState();
			case YURT_WALL_OUTER: 	return Content.YURT_WALL_OUTER.getDefaultState();
			case YURT_ROOF: 		return Content.YURT_ROOF.getDefaultState();
			case TEPEE_WALL: 		return Content.TEPEE_WALL.getDefaultState();
			case BEDOUIN_WALL:		return Content.BEDOUIN_WALL.getDefaultState();
			case BEDOUIN_ROOF:		return Content.BEDOUIN_ROOF.getDefaultState();
			}
			return null;
		}
	}
